...
TraktorS2MK3.registerInputPackets = function () {
    ...
    this.registerInputScaler(messageLong, "[QuickEffectRack1_[Channel1]]", "super1", 0x13, 0xFFFF, this.parameterHandler); //quickEffectHandler);
    this.registerInputScaler(messageLong, "[QuickEffectRack1_[Channel2]]", "super1", 0x25, 0xFFFF, this.parameterHandler); //quickEffectHandler);
...
TraktorS2MK3.activateLoopHandler = function (field) {
    ...
    if (TraktorS2MK3.shiftPressed[field.group]) {
        ...
        var loop_enabled = engine.getValue(field.group, "loop_enabled");
        if (loop_enabled){
            engine.setValue(field.group, "reloop_toggle", field.value);
        } else {
...
TraktorS2MK3.jogTouchHandler = function (field) {
    ...
    var fast_seek = 1;
    if (TraktorS2MK3.shiftPressed[field.group]) { fast_seek = 100; }
    if (field.value > 0) {
        engine.scratchEnable(deckNumber, 1024 / fast_seek, 33 + 1 / 3, 0.125, 0.125 / 8, true);
    } else {
...
TraktorS2MK3.quickEffectHandler = function (field) {
    
    //QuickEffectRack1_[Channel1]]
    var channel = field.group.substring(17,26)
    
    if(TraktorS2MK3.fxButtonState[1]){
    // only do default (filter) if 1 is set
        engine.setParameter(field.group, field.name, field.value / 4095);
    }
    if(TraktorS2MK3.fxButtonState[2]){
    // only do default (filter) if 1 is set
        engine.setParameter(field.group, field.name, field.value / 4095);
    }
    if(TraktorS2MK3.fxButtonState[3]){
    // only do default (filter) if 1 is set
        engine.setParameter(field.group, field.name, field.value / 4095);
    }
    if(TraktorS2MK3.fxButtonState[4]){
    // only do default (filter) if 1 is set
        engine.setParameter(field.group, field.name, field.value / 4095);
    }
